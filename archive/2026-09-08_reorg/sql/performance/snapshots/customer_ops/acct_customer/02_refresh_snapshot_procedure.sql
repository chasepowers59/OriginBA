...
BEGIN
    -- Retention: drop dormant accounts set up before the window with no recent
    -- SA or alert activity and no currently open alerts.
    DELETE FROM cisadm.acct_customer_rpt_curr snap
    WHERE snap.setup_dt < v_window_start
      AND NOT EXISTS (
          SELECT 1
          FROM cisadm.ci_sa sa
          WHERE sa.acct_id = snap.acct_id
            AND (
                sa.start_dt >= v_window_start
                OR sa.end_dt >= v_window_start
      ...
      AND NOT EXISTS (
          ...
          WHERE al.acct_id = snap.acct_id
            AND (
                al.start_dt >= v_window_start
                OR al.end_dt >= v_window_start
                OR (
    ...
    -- Replace the refresh scope so account-level aggregates stay current.
    DELETE FROM cisadm.acct_customer_rpt_curr snap
    WHERE EXISTS (
        SELECT 1
        FROM cisadm.ci_acct acct
        WHERE acct.acct_id = snap.acct_id
          AND (
              acct.setup_dt >= v_window_start
              OR EXISTS (
                  SELECT 1
                  FROM cisadm.ci_sa sa
    ...
    refresh_scope AS (
        SELECT acct.acct_id
        FROM cisadm.ci_acct acct
        WHERE acct.setup_dt >= v_window_start
        UNION
        SELECT sa.acct_id
        FROM cisadm.ci_sa sa
        WHERE sa.start_dt >= v_window_start
           OR sa.end_dt >= v_window_start
           OR sa.sa_status_flg = '20'
        UNION
        SELECT al.acct_id
        FROM cisadm.ci_acct_alert al
        WHERE al.start_dt >= v_window_start
           OR al.end_dt >= v_window_start
           OR (
               TRUNC(SYSDATE) >= al.start_dt
               AND (al.end_dt IS NULL OR TRUNC(SYSDATE) <= al.end_dt)
