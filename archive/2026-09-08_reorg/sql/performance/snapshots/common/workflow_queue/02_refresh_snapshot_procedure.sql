...
BEGIN
    DELETE FROM cisadm.workflow_queue_rpt_curr snap
    WHERE snap.queue_source = 'TODO'
      AND snap.queue_anchor_dttm < v_window_start
    ...
    WHERE snap.queue_source = 'TODO'
      AND (
          snap.queue_anchor_dttm >= v_window_start
          OR snap.entry_status_flg = 'O'
      );
    COMMIT;
    ...
    WHERE snap.queue_source = 'BATCH'
      AND (
          snap.queue_anchor_dttm >= v_window_start
          OR snap.batch_end_dttm IS NULL
      );
    COMMIT;
    ...
    todo_rows AS (
        ...
        WHERE td.cre_dttm >= v_window_start
           OR td.entry_status_flg = 'O'
    ),
    batch_rows AS (
        SELECT
        ...
        WHERE inst.start_dttm >= v_window_start
           OR run.end_dttm IS NULL
    )
    SELECT
        src.*,
        v_load_dttm
