...
-- Read-only gate: returns PASS row and raises SQL error if any per-class diff is non-zero.
-- Required vars:
...
SELECT
  'PER_CLASS_ZERO_DIFF_GATE' AS gate_name,
...
-- Force SQL*Plus failure if mismatches exist (still read-only).
WITH original_agg AS (
  SELECT
    ci_cust_cl_l.descr AS customer_class,
    NVL(SUM(dscalar.quantity),0) AS original_total
...
SELECT 1 / CASE WHEN SUM(CASE WHEN difference <> 0 THEN 1 ELSE 0 END) = 0 THEN 1 ELSE 0 END AS assert_zero_diff
FROM diffs;
