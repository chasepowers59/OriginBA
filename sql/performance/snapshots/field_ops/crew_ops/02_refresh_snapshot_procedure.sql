...
BEGIN
    DELETE FROM cisadm.crew_ops_rpt_curr
    WHERE crew_id IN (
        SELECT DISTINCT ch.srch_char_val
    ...
    WITH
    touched_crew AS (
        SELECT DISTINCT ch.srch_char_val AS crew_id
        FROM cisadm.d1_activity_char ch
        INNER JOIN cisadm.d1_activity act
            ON act.d1_activity_id = ch.d1_activity_id
        INNER JOIN cisadm.d1_activity_type act_type
    ...
    fa_crew_link AS (
        SELECT
            ...
            END AS open_fa_days
        FROM cisadm.d1_activity_char ch
        INNER JOIN touched_crew tc
            ON tc.crew_id = ch.srch_char_val
        INNER JOIN cisadm.d1_activity act
            ON act.d1_activity_id = ch.d1_activity_id
        INNER JOIN cisadm.d1_activity_type act_type
            ON act_type.activity_type_cd = act.activity_type_cd
    ...
    SELECT
        ...
        v_load_dttm
    FROM touched_crew tc
    INNER JOIN cisadm.c1_representative rep
        ON rep.c1_representative_cd = tc.crew_id
