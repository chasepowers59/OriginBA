    ...
    LOOP
        ...
        WITH eligible_bseg AS (
            ...
            INNER JOIN cisadm.ci_bill bill
                ON bill.bill_id = bseg.bill_id
               AND bill.bill_stat_flg = 'C '
               AND bill.bill_dt >= v_batch_start
               AND bill.bill_dt < v_batch_end
        ),
        eligible_acct AS (
            SELECT /*+ materialize */
                   DISTINCT
                   bill.acct_id
            FROM eligible_bseg eb
            INNER JOIN cisadm.ci_bseg bseg
                ON bseg.bseg_id = eb.bseg_id
            INNER JOIN cisadm.ci_bill bill
                ON bill.bill_id = bseg.bill_id
        ),
        cust_name AS (
            ...
            FROM (
                ...
                INNER JOIN eligible_acct ea
                    ON ea.acct_id = ap.acct_id
        ...
        sq_agg AS (
            ...
            INNER JOIN eligible_bseg eb
                ON eb.bseg_id = sq.bseg_id
            GROUP BY
                sq.bseg_id
        ),
        read_agg AS (
