CREATE OR REPLACE PROCEDURE cisadm.refresh_device_sp_rpt_curr AS
    ...
    v_window_start   TIMESTAMP := CAST(ADD_MONTHS(TRUNC(SYSDATE), -6) AS TIMESTAMP);
BEGIN
    DELETE FROM cisadm.device_sp_rpt_curr snap
    WHERE snap.dvc_status_upd_dttm < v_window_start
      ...
      AND NOT EXISTS (
          ...
          WHERE cfg.d1_device_id = snap.d1_dvc_id
            AND (
                ie.d1_install_dttm >= v_window_start
                OR ie.d1_removal_dttm >= v_window_start
                OR ie.status_upd_dttm >= v_window_start
            )
      )
      AND NOT EXISTS (
          SELECT 1
          ...
          WHERE cfg.d1_device_id = snap.d1_dvc_id
            AND (
                cfg.eff_dttm >= v_window_start
                OR cfg.status_upd_dttm >= v_window_start
            )
      );
    COMMIT;
    ...
    WHERE EXISTS (
        ...
        WHERE dvc.d1_device_id = snap.d1_dvc_id
          AND (
              dvc.status_upd_dttm >= v_window_start
              OR dvc.cre_dttm >= v_window_start
              OR EXISTS (
                  SELECT 1
                  FROM cisadm.d1_dvc_cfg cfg
                  JOIN cisadm.d1_install_evt ie
                      ON ie.device_config_id = cfg.device_config_id
                  WHERE cfg.d1_device_id = dvc.d1_device_id
                    AND (
                        ie.d1_install_dttm >= v_window_start
                        OR ie.d1_removal_dttm >= v_window_start
                        OR ie.status_upd_dttm >= v_window_start
              ...
              OR EXISTS (
                  ...
                  JOIN cisadm.d1_install_evt ie
                      ON ie.device_config_id = cfg.device_config_id
                  WHERE cfg.d1_device_id = dvc.d1_device_id
                    AND (ie.d1_install_dttm IS NULL OR ie.d1_install_dttm <= SYSTIMESTAMP)
    ...
    WITH
    refresh_scope AS (
        SELECT dvc.d1_device_id
        FROM cisadm.d1_dvc dvc
        WHERE dvc.status_upd_dttm >= v_window_start
           OR dvc.cre_dttm >= v_window_start
        UNION
        SELECT cfg.d1_device_id
        FROM cisadm.d1_dvc_cfg cfg
        JOIN cisadm.d1_install_evt ie
            ON ie.device_config_id = cfg.device_config_id
        WHERE ie.d1_install_dttm >= v_window_start
           OR ie.d1_removal_dttm >= v_window_start
           OR ie.status_upd_dttm >= v_window_start
        UNION
        SELECT cfg.d1_device_id
        FROM cisadm.d1_dvc_cfg cfg
        WHERE cfg.eff_dttm >= v_window_start
           OR cfg.status_upd_dttm >= v_window_start
        UNION
        SELECT cfg.d1_device_id
        FROM cisadm.d1_dvc_cfg cfg
    ...
    cfg_agg AS (
        ...
        JOIN refresh_scope rs
            ON rs.d1_device_id = cfg.d1_device_id
    ...
    FROM cisadm.d1_dvc dvc
    JOIN refresh_scope rs
        ON rs.d1_device_id = dvc.d1_device_id
