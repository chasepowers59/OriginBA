 ...
 *
 * The page told everyone "Query the CISADM schema you know from CIS", which is right for
 * the legacy Oracle path and incomplete everywhere else: both dbt engines reach CISADM
 * AND the reporting canvases beside it. Measured against the running fence rather than
 * assumed — a Postgres org's rejection message names its own scope, "The workspace is
 * scoped to cisadm, reporting", with staging and core correctly refused as internal
 * build layers.
 *
