    ...
    <div className="glass-panel overflow-hidden">
      ...
      {expanded ? (
        <div className="border-t border-edge-subtle px-5 pb-5">
          {error ? (
            <p className="mt-4 rounded-lg border border-warn bg-warn-bg px-3 py-2 text-sm text-warn">
          ...
          {data?.rows.length ? (
            <div className="mt-4 space-y-4">
              {data.rows.map((row, idx) => (
                ...
                >
                  <p className="mb-3 text-xs font-medium text-primary">Sample row {idx + 1}</p>
                  <dl className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                    {data.columns.map((col) => (
                      <div key={col} className="min-w-0">
                        <dt className="truncate text-[10px] uppercase tracking-wide text-fg-muted">
                          {data.column_labels[col] ?? prettifyFieldName(col)}
                        </dt>
                        <dd className="truncate text-sm text-heading" title={String(row[col] ?? "")}>
