...
> **Resource roots (2026-09-08).** Authoring exports use `/organizations/organization_1/organizations/Origin_DEV/SmartCity/...`; client tenant imports are tenant-relative `/SmartCity/...` with no `rootTenantId` (`scripts/jaspersoft/tenant_root_layout.py`); `Standard_Offering/` is a folder under either root. These are three pipeline stages, not three competing conventions. The loadable, script-backed contract is `.claude/skills/jaspersoft-client-tenant-import/SKILL.md`; the client list and DataSource names come from `~/originba_dbt/clients.yml`.
