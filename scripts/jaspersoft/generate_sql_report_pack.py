...
ORDER BY BILLED_AMT DESC""",
        ...
        filters=[
            Filter("BILL_CYC_CD_F", "Bill cycles",
                   ...
                   lov_sql="SELECT TRIM(bill_cyc_cd) AS CODE, TRIM(bill_cyc_cd) || ' - ' || descr AS DESCR FROM CISADM.CI_BILL_CYC_L l WHERE l.language_cd = 'ENG' AND EXISTS (SELECT 1 FROM CISADM.CI_BILL b WHERE b.bill_cyc_cd = l.bill_cyc_cd) ORDER BY 1"),
            Filter("SVC_TYPE_CD_F", "Service type",
                   ...
                   lov_sql="SELECT TRIM(svc_type_cd) AS CODE, TRIM(svc_type_cd) || ' - ' || descr AS DESCR FROM CISADM.CI_SVC_TYPE_L l WHERE l.language_cd = 'ENG' AND EXISTS (SELECT 1 FROM CISADM.CI_SA_TYPE t JOIN CISADM.CI_SA sa ON sa.sa_type_cd = t.sa_type_cd AND sa.cis_division = t.cis_division WHERE t.svc_type_cd = l.svc_type_cd) ORDER BY 1"),
            Filter("CIS_DIVISION_F", "CIS division",
                   ...
                   lov_sql="SELECT TRIM(cis_division) AS CODE, TRIM(cis_division) || ' - ' || descr AS DESCR FROM CISADM.CI_CIS_DIVISION_L l WHERE l.language_cd = 'ENG' AND EXISTS (SELECT 1 FROM CISADM.CI_ACCT a WHERE a.cis_division = l.cis_division) ORDER BY 1"),
...
ORDER BY PAY_MONTH""",
        ...
        filters=[
            Filter("TENDER_TYPE_CD_F", "Tender types",
                   ...
                   lov_sql="SELECT TRIM(tender_type_cd) AS CODE, TRIM(tender_type_cd) || ' - ' || descr AS DESCR FROM CISADM.CI_TENDER_TYPE_L l WHERE l.language_cd = 'ENG' AND EXISTS (SELECT 1 FROM CISADM.CI_PAY_TNDR t WHERE t.tender_type_cd = l.tender_type_cd) ORDER BY 1"),
...
FETCH FIRST $P{{TOP_N}} ROWS ONLY""",
        ...
        filters=[
            Filter("ADJ_TYPE_CD_F", "Adjustment type",
                   ...
                   "($P{ADJ_TYPE_CD_F} IS NULL OR TRIM(a.adj_type_cd) = TRIM($P{ADJ_TYPE_CD_F}))",
                   lov_sql="SELECT TRIM(adj_type_cd) AS CODE, TRIM(adj_type_cd) || ' - ' || descr AS DESCR FROM CISADM.CI_ADJ_TYPE_L l WHERE l.language_cd = 'ENG' AND EXISTS (SELECT 1 FROM CISADM.CI_ADJ a WHERE a.adj_type_cd = l.adj_type_cd) ORDER BY 1"),
            Filter("ACCT_ID_F", "Account ID",
...
ORDER BY ACCT_MONTH""",
        ...
        filters=[
            Filter("DST_ID_F", "Distribution code",
                   ...
                   "($P{DST_ID_F} IS NULL OR TRIM(g.dst_id) = TRIM($P{DST_ID_F}))",
                   lov_sql="SELECT TRIM(dst_id) AS CODE, TRIM(dst_id) || ' - ' || descr AS DESCR FROM CISADM.CI_DST_CODE_L l WHERE l.language_cd = 'ENG' AND EXISTS (SELECT 1 FROM CISADM.CI_FT_GL g WHERE g.dst_id = l.dst_id) ORDER BY 1"),
            ...
            Filter("GL_DIVISION_F", "GL division",
                   ...
                   "($P{GL_DIVISION_F} IS NULL OR TRIM(f.gl_division) = TRIM($P{GL_DIVISION_F}))",
                   lov_sql="SELECT TRIM(gl_division) AS CODE, TRIM(gl_division) || ' - ' || descr AS DESCR FROM CISADM.CI_GL_DIVISION_L l WHERE l.language_cd = 'ENG' AND EXISTS (SELECT 1 FROM CISADM.CI_FT f WHERE f.gl_division = l.gl_division) ORDER BY 1"),
            Filter("CIS_DIVISION_F", "CIS division",
            ...
            Filter("FT_TYPE_FLG_F", "FT type (BS, BX, AD, AX, PS, PX)",
                   ...
                   "($P{FT_TYPE_FLG_F} IS NULL OR TRIM(f.ft_type_flg) = TRIM($P{FT_TYPE_FLG_F}))",
                   lov_sql="SELECT TRIM(field_value) AS CODE, TRIM(field_value) || ' - ' || descr AS DESCR FROM CISADM.CI_LOOKUP_VAL_L l WHERE l.field_name = 'FT_TYPE_FLG' AND l.language_cd = 'ENG' AND EXISTS (SELECT 1 FROM CISADM.CI_FT f WHERE f.ft_type_flg = l.field_value) ORDER BY 1"),
        ]),
]
